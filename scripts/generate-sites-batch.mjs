/**
 * Generate 5 Vertxia Lite sites en batch.
 *
 * Pipeline : scrape Shopify products.json -> brief Claude sonnet -> save data/briefs.
 *
 * Run : cd web && node scripts/generate-sites-batch.mjs
 */
...
if (!process.env.ANTHROPIC_API_KEY) {
  console.error("ANTHROPIC_API_KEY absent — arret.");
  process.exit(1);
}
...
const TARGETS = [
  {
    domain: "bombas.com",
    prompt: "site lifestyle warm, manifeste sur le confort et la generosite, vibe accessible premium",
  },
  {
    domain: "fellowproducts.com",
    prompt: "rituel cafe matinal, premium minimal industrial, cinematic objet design",
  },
  {
    domain: "magicspoon.com",
    prompt: "pop punchy joyeux, manifeste contre les cereales chimiques, palette saturee assumee",
  },
  {
    domain: "ouai.com",
    prompt: "editorial salon parisien moderne, sensualite des cheveux, vibe Glossier mais plus mature",
  },
  {
    domain: "offlimits.com",
    prompt: "manifeste cereal-as-art, NYC artsy chaotique, brutaliste joyeux retro 90s",
  },
];
...
const BRIEF_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      creative_direction: {
        type: "object",
        properties: { mood: { type: "string" }, reference_style: { type: "string" }, narrative_arc: { type: "string" } },
      ...
      visual_system: {
        ...
        properties: {
          palette: {
            ...
            items: { type: "object", properties: { name: { type: "string", enum: ["background", "foreground", "accent", "muted", "surface"] }, hex: { type: "string", pattern: "^#[0-9A-Fa-f]{6}$" } }, required: ["name", "hex"] },
          },
      ...
      hero: {
        type: "object",
        properties: { kicker: { type: "string" }, headline: { type: "string" }, subheadline: { type: "string" }, primary_cta_label: { type: "string" }, secondary_cta_label: { type: ["string", "null"] } },
        required: ["kicker", "headline", "subheadline", "primary_cta_label"],
      },
      site_structure: {
        type: "array", minItems: 3, maxItems: 5,
        items: { type: "object", properties: { section: { type: "string" }, section_role: { type: "string" }, headline: { type: "string" }, body_paragraphs: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 4 }, pull_quote: { type: ["string", "null"] } }, required: ["section", "section_role", "headline", "body_paragraphs"] },
      },
      featured_products: {
        type: "array", minItems: 3, maxItems: 6,
        items: { type: "object", properties: { handle: { type: "string" }, title: { type: "string" }, hero_image_url: { type: "string" }, price_eur: { type: ["string", "number", "null"] }, editorial_caption: { type: "string" }, video_prompt: { type: "string" }, video_engine_hint: { type: "string", enum: ["kling", "runway", "veo", "higgsfield", "hailuo"] }, video_duration_s: { type: "number", enum: [5] } }, required: ["handle", "title", "hero_image_url", "editorial_caption", "video_prompt", "video_engine_hint", "video_duration_s"] },
      },
...
/* ----------- Scraper Shopify minimal (port du web/lib/pipeline/scraper.ts) ----------- */
...
Appelle submit_brief.`;
  ...
  if (!toolUse) {
    throw new Error("Claude n'a pas appele submit_brief");
  }

  // Inject _meta pour matcher le format CreativeBrief
  return {
    ...toolUse.input,
    _meta: {
      model: "claude-sonnet-4-6",
      ...
      featured_count: toolUse.input.featured_products?.length || 0,
...
/* ----------- Inject scraped image URLs dans le brief ----------- */
...
async function processOne(target, client) {
  ...
  try {
    ...
    return { slug, domain: target.domain, status: "scrape_failed", error: err.message };
  }

  if (scrape.products.length === 0) {
    console.error(`  scrape KO : 0 produits`);
    return { slug, domain: target.domain, status: "no_products" };
  }
  ...
  try {
    ...
    console.log(`    mood : ${brief.creative_direction?.mood?.slice(0, 90)}`);
    console.log(`    palette : ${brief.visual_system?.palette?.map(p => p.hex).join(' ')}`);
  } catch (err) {
    console.error(`  brief KO : ${err.message}`);
    return { slug, domain: target.domain, status: "brief_failed", error: err.message };
  }
  ...
  return {
    ...
    signature: brief.visual_signature,
    palette: brief.visual_system?.palette?.map((p) => p.hex),
    mood: brief.creative_direction?.mood,
  };
}
...
for (const target of TARGETS) {
  const r = await processOne(target, client);
  results.push(r);
}
...
for (const r of results) {
  if (r.status === "ok") {
    console.log(`OK  ${r.domain.padEnd(28)} -> /lite/${r.slug}  [${r.template}]`);
  } else {
    console.log(`KO  ${r.domain.padEnd(28)} -> ${r.status}: ${r.error || ""}`);
  }
}

const okCount = results.filter((r) => r.status === "ok").length;
console.log(`\n${okCount}/${results.length} sites OK`);
