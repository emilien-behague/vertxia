...
//
// Reprend exactement le pattern du composant <VoiceInput /> mais en
// modal plein écran avec affichage XL de la transcription.
...
type Props = {
  open: boolean;
  onClose: () => void;
  onExtraction: (result: ExtractionResult, transcript: string) => void;
  equipementContext?: {
    modele?: string;
    fluide?: string;
    chargeKg?: number;
...
export function VoiceFullDictation({ open, onClose, onExtraction, equipementContext }: Props) {
  ...
  function startListening() {
    ...
    recognition.onend = () => {
      // L'événement onend arrive aussi quand on stop manuellement.
      // On NE change PAS la phase ici — c'est handleStop() qui s'en occupe.
    };
  ...
  async function handleStop() {
    stopRecognition();
    // Finaliser le transcript avec l'interim au cas où
    ...
    try {
      const res = await fetch("/api/voice/extract-intervention", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ transcript: finalText, equipementContext }),
      });
      ...
      const data = (await res.json()) as { extraction: ExtractionResult; transcript: string };
      onExtraction(data.extraction, finalText);
      // Le parent ferme le modal après avoir appliqué l'extraction
    } catch (err) {
  ...
  return (
    ...
    >
        ...
        {phase === "idle" && (
          <div className="max-w-md mx-auto pt-8">
            <p className="text-[18px] leading-relaxed text-white/85 mb-6">
              Dicte ton intervention en langage naturel. L&apos;IA range tout dans les bonnes cases du CERFA.
            </p>
            <div className="rounded-2xl bg-white/[0.06] p-5 mb-6">
              ...
              <p className="text-[14px] text-white/65 leading-relaxed italic">
                «&nbsp;Maintenance annuelle sur la PAC Daikin, étanchéité OK rien à signaler, j&apos;ai rajouté 200 grammes de R32 sur le circuit. Le client c&apos;est Martin Dupont, j&apos;interviens chez lui à Toulon.&nbsp;»
              </p>
            </div>
      ...
      {/* Footer — bouton action principal */}
